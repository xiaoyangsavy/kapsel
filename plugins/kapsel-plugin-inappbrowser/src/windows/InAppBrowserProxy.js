...
var IAB = {
    ...
    open: function (win, lose, args) {
        ...
        if (target === "_system") {
            var appdata = 'ms-appdata:///local/';
            //In case the file is opened from the local app data directory
            if (strUrl.indexOf(appdata) == 0) {
                //Get the filename
                var fileName = decodeURI(strUrl.substr(String(strUrl).lastIndexOf("/") + 1));
                //Get the local folder which is c:\Users\<username>\AppData\Local\Packages\<appId>\LocalState\
                var localFolder = Windows.Storage.ApplicationData.current.localFolder;
                //Get the directory if it exists on a way that remove the appdata, the filename and change the slash to backslash
                var directory = strUrl.replace(appdata, '').replace(fileName, '').replace(/\//g, '\\');
                //If there is no subdirectory then open the file
                if (directory.length == 0) {
                    ...
                    //otherwise open the given subdirectory or more of them and open the file from there
                    localFolder.getFolderAsync(directory).then(function (dir) {
                        ...
                        console.log("Error getting directory '" + directory + "': " + error);
                    });
                }
            } else {
