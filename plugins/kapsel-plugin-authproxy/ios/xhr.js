(function _wk_xhr_proxy() {
  ...
  XHRPrototype.open = function _wk_open(method, url, async, user, password) {
  ...
  XHRPrototype.send = function _wk_send() {
    if (this.__fakeData) {
      ...
      var async = this.__get('async');
      if (async) {
        return scheduleXHRRequest(this, this.__getURL());
      } else {
        return scheduleSyncXHRRequest(this, this.__getURL());
      }
    }
  ...
  function scheduleSyncXHRRequest(context, url) {
    var responseMsg = prompt("gap_xhr:", url);
    var message = JSON.parse(responseMsg);
    if (message.status === '200') {
      context.__set('readyState', 4);
      context.__set('status', 200);
      context.__set('statusText', 'OK');

      var buffer = decodeBase64(message.responseText);

      switch (context.__get('responseText')) {
        case 'arraybuffer':
          ...
          return buffer;
        default:
          console.error('Unknown responseText:', this.responseText);
        case 'text':
        case '':
